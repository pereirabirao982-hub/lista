...
export default function HomePage() {
  ...
  return (
    <div className="paper-grain min-h-[100dvh] overflow-hidden bg-background">
      <SiteHeader />
      <main>
        <section className="relative mx-auto grid max-w-7xl gap-8 px-5 pb-16 pt-8 sm:px-8 lg:grid-cols-[1.05fr_.95fr] lg:items-center lg:px-10 lg:pb-28 lg:pt-14">
          <div className="relative z-10 animate-rise-in">
            ...
            <h1 className="serif max-w-3xl text-[clamp(3.2rem,8vw,7.6rem)] font-semibold leading-[.91] tracking-[-.06em] text-primary">A melhor parte<br /><em className="font-medium text-accent">é você vir.</em></h1>
            <p className="mt-7 max-w-md text-base leading-relaxed text-primary/65 sm:text-lg" data-testid="text-event-subtitle">{event.subtitle}</p>
            <div className="mt-8 flex flex-col items-stretch gap-3 sm:flex-row sm:items-center">
              <Link href="/sign-up" className="group flex min-h-12 items-center justify-center gap-3 rounded-full bg-primary px-6 py-3.5 text-sm font-bold text-primary-foreground shadow-md transition-transform duration-300 hover:-translate-y-0.5" data-testid="link-guest-sign-up">Confirmar presença <ArrowRight size={16} className="transition-transform group-hover:translate-x-1" /></Link>
              <Link href="/sign-in" className="flex min-h-12 items-center justify-center rounded-full border border-primary/20 px-6 py-3.5 text-sm font-bold text-primary transition-colors hover:border-primary/50" data-testid="link-guest-sign-in">Já tenho acesso</Link>
            </div>
            <p className="mt-4 text-xs text-primary/45">Leva menos de dois minutos. E você escolhe um presente especial.</p>
          </div>
          <div className="relative min-h-[350px] animate-rise-in delay-2 sm:min-h-[480px]">
            <div className="absolute left-[9%] top-[6%] size-[74%] rounded-[48%_52%_55%_45%/42%_41%_59%_58%] bg-secondary/80 animate-float-slow" />
            <div className="absolute right-0 top-0 w-[86%] max-w-[410px] rounded-[2rem] border border-primary/10 bg-card p-6 shadow-xl sm:p-9">
              <div className="flex items-start justify-between"><span className="mono text-[10px] uppercase tracking-[.18em] text-accent">O encontro</span><span className="grid size-8 place-items-center rounded-full bg-accent/20 text-accent"><CalendarDays size={15} /></span></div>
               <p className="serif mt-10 text-3xl font-semibold leading-[.98] sm:mt-14 sm:text-4xl" data-testid="text-event-title">{event.title}</p>
               <div className="mt-9 space-y-5 border-t border-border pt-5 text-sm sm:mt-12">
            ...
            <div className="absolute bottom-0 left-0 flex w-[50%] max-w-[210px] -rotate-6 flex-col gap-3 rounded-2xl bg-accent p-4 text-primary shadow-lg sm:bottom-5 sm:left-4 sm:p-5">
