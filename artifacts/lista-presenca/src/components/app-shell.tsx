  ...
  return (
    <header className="relative z-20 mx-auto flex w-full max-w-7xl items-center justify-between px-6 py-6 sm:px-10 lg:px-12 lg:py-8">
      ...
      <nav className="hidden items-center gap-8 text-sm font-medium md:flex" aria-label="Navegação principal">
        ...
        <ThemeToggle compact />
        <Link href="/admin" className="rounded-full border border-primary/20 px-5 py-2.5 text-primary transition-all hover:border-primary/50 hover:bg-primary/5" data-testid="link-header-admin">Área administrativa</Link>
      </nav>
    ...
    <div className="relative">
      ...
      {open && (
        <div className="absolute right-0 top-16 z-30 w-[min(16rem,calc(100vw-3rem))] rounded-2xl border border-border bg-card p-3 shadow-xl animate-fade-in-up">
          ...
          <Link href="/admin" onClick={() => setOpen(false)} className="flex min-h-12 items-center rounded-xl px-4 text-sm font-medium text-accent hover:bg-accent/10 transition-colors" data-testid="link-mobile-admin">Área administrativa</Link>
        </div>
      )}
    </div>
  );
...
export function AppShell({ children, admin = false, guestName }: { children: ReactNode; admin?: boolean; guestName?: string }) {
  const { user } = useUser();
  const { signOut } = useClerk();
  const [navOpen, setNavOpen] = useState(false);
  const name = guestName || user?.firstName || user?.emailAddresses[0]?.emailAddress?.split('@')[0] || 'Convidado';
      ...
      <aside className={`fixed inset-y-0 left-0 z-40 flex w-[280px] flex-col border-r border-sidebar-border bg-sidebar px-6 py-8 text-sidebar-foreground transition-transform duration-500 cubic-bezier(0.16,1,0.3,1) lg:translate-x-0 ${navOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="mt-16">
          ...
          <nav className="space-y-1.5" aria-label="Navegação do espaço">
            {!admin && <Link href="/user-portal" className="flex items-center gap-3.5 rounded-xl bg-sidebar-accent px-4 py-3.5 text-sm font-medium text-sidebar-foreground shadow-sm" data-testid="link-sidebar-portal">
        ...
        <div className="mt-auto pt-8">
          ...
          <div className="flex items-center gap-4 px-3">
            ...
            <div className="min-w-0">
              ...
              <p className="truncate text-xs text-sidebar-foreground/60">{admin ? 'Administrador' : 'Convidado especial'}</p>
            </div>
          </div>
          {admin && <button type="button" onClick={() => signOut({ redirectUrl: import.meta.env.BASE_URL.replace(/\/$/, '') || '/' })} className="mt-6 flex w-full items-center gap-3.5 rounded-xl border border-transparent px-4 py-3 text-sm font-medium text-sidebar-foreground/70 transition-colors hover:border-sidebar-border hover:bg-sidebar-accent/30 hover:text-sidebar-foreground" data-testid="button-sign-out">
